...
- 重命名的测试以及debug   
- 和上传到仓库的git协作起来  
- 目前是在window10上进行的测试，后续更新跨平台的支持  
- 建立 序列化和反序列化：将扫描到的文件信息序列化为二进制或其他格式，可以保存到磁盘上，以便下次使用时直接反序列化，避免再次扫描磁盘。可以使用JSON、XML、Protocol Buffers等格式进行序列化。      
...
- 修改区域设置，对Beta版本选中，支持UTF-8语言，否则乱码  

- 添加*.mp3无法修改json文件的bug  

- 拷贝代码空格太多的问题  

- 在大小窗口切换时，需要恢复放缩之前的大小 

- 添加创建仓库功能  

- 修复了添加assetstype添加Item的bug 

- 敲回车相当于确定    

- 适配不同分辨率的窗口大小    

- 需要将快捷键 Ctrl 和 Alt 的分一下组，容易记忆 

- 界面需要优化和设计一下 

- 托盘的图标可以切换  

- 添加和修改资源文件选项，添加和修改选取资源的类型，可以设置资源类型： 比如添加 MP3等其他资源  →  可以通过配置界面实现整体配置      

- 添加创建文件时，编号-(不可编辑) + 文件名(可以编辑)  +  模板选择(默认是00模板)  

- 添加title显示当前文件名功能   

- 添加 MP3 资源的插入： 行插入和嵌入文字插入     

- <p>输入A文本&nbsp;<audio src=http://dict.youdao.com/dictvoice?type=0&audio=buoyant style="width:120px; height: 14px"/></audio> &nbsp;输入B文本</p> 
- 将**50% → 49%** ，将 **25% → 32%**  
- 添加repo菜单功能
